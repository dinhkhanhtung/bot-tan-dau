...
async function handleUnregisteredSpam(facebookId: string, message: string, userStatus: string): Promise<{
    ...
    // Xử lý theo level - DISABLED WELCOME LOGIC TO AVOID DUPLICATES
    if (newCount === 1) {
        // DISABLED - Welcome is now handled by WelcomeService only
        console.log('🎉 First message - welcome handled by WelcomeService')
        return { action: 'none', block: false, message: 'Welcome handled by WelcomeService' }
    } else if (newCount >= 2) {
...
export async function trackNonButtonMessage(facebookId: string, message: string): Promise<{
    ...
    // Admin chat functionality removed - always return false
    const isInAdminChat = false
